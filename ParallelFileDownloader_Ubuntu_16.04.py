...
def create_connection(url,data_capacity_of_each_thread,last_read_byte,upper_limit):
    ...
    range_header = "Range: bytes = {}-{}".format(last_read_byte,upper_limit)
...
def get_request_msg(target_download_url, request_type="GET", custom_header=""):
    msg = '{} /{} HTTP/1.1\r\nHost:%s\r\n\r\n'.format(request_type,target_download_url[target_download_url.find("/"):]) % target_download_url[:target_download_url.find("/")]
    if custom_header != "":
        msg += custom_header + '\r\n'
        msg += '\r\n'
    return msg
...
# Make a GET request
# Get  and save it to
print("URL of the index file: {} \nNumber of parallel connections: {}\nIndex file is downloaded.".format(target_url,thread_counter))
...
try:
    ...
    print("{} could not founded ...".format(target_url))
    print("Program will exit.")
    sys.exit(1)

print("There are {} files in the index. ".format(len(url_list)))
...
for x in url_list:
    ...
    if splitted[0] == ('HTTP/1.1 404 Not Found'):
        print("{}. {} not found. ".format(str(counter),x))
    else:
        last_read_byte = 0
        upper_limit = 0
        file_size = get_size_of_file(x)
        global result
        result = "{}. {} (size = {}) is downloaded \n File parts: ".format(str(counter),x,file_size)
        if(file_size == -1):
            result = "{}. {} (size = 0) is downloaded.".format(str(counter),x)
            ...
            if file_size % thread_counter == 0:
                ...
                for y in range(thread_counter):
                    ...
                    result += "{}:{}({})  ".format(int(last_read_byte - data_capacity_of_each_thread),int(last_read_byte),int(data_capacity_of_each_thread))
